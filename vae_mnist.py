...
if __name__ == '__main__':
    ...
    epochs = 15  # 15轮 训练
    batch_size = 128 # 每一轮中每次训练单次投入的数据
    learning_rate = 1e-3
    N_TEST_IMG = 5 # 展示的图片

    # 加载数据
    dataset = datasets.MNIST('./',train=True,transform=transforms.ToTensor(),
                                         download=True)
    ...
    for epoch in range(epochs):
        for epochs_index, (train_data, _) in enumerate(data_loader):
            ...
            if (epochs_index + 1) % 100 == 0:
                print("Epoch[{}/{}], Step [{}/{}], Reconst Loss: {:.4f}, KL Div: {:.4f}"
                      .format(epoch + 1, epochs, epochs_index + 1, len(data_loader), reconst_loss.item(),kl_div.item()))
